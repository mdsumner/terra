...
setMethod("project", signature(x="SpatRaster"),
	function(x, y, method, mask=FALSE, align=FALSE, gdal=TRUE, res=NULL, origin=NULL, threads=FALSE, filename="", ..., by_util = FALSE)  {
		...
		if (inherits(y, "SpatRaster")) {
			if (gdal) {
				if (by_util) {
						x@pnt <- x@ptr$warp_by_util(y@ptr, "", method, mask[1], align[1], FALSE, opt)
				} else {
					x@pnt <- x@ptr$warp(y@ptr, "", method, mask[1], align[1], FALSE, opt)
				}
			} else {
				if (align) {
			...
			if (gdal) {
				if (by_util) {
					x@pnt <- x@ptr$warp_by_util(SpatRaster$new(), y, method, mask, FALSE, FALSE, opt)
					
				} else {
					x@pnt <- x@ptr$warp(SpatRaster$new(), y, method, mask, FALSE, FALSE, opt)
				}
			} else {
				y <- project(rast(x), y)
				x@pnt <- x@pnt$resample(y@pnt, method, mask[1], TRUE, opt)
